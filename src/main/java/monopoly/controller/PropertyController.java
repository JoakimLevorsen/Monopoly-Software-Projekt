...
public class PropertyController {
	...
	public void auction(StationSpace property) {
		gooey.showMessage(jsonData.getString(JSONKey.STATION.getKey()) + property.getName()
				+ jsonData.getString(JSONKey.AUCTIONED_OFF.getKey()));
		...
		while ((noBidCounter < players.size() - 1 && topBid != 0) || noBidCounter < players.size()) {
			...
			int bid = gooey.getUserInteger(
					jsonData.getString(JSONKey.BID.getKey()) + (currentBidderIndex + 1)
							+ jsonData.getString(JSONKey.ANSWER_TO_BID.getKey()) + topBid,
		...
		if (topBidder != null) {
			gooey.showMessage(jsonData.getString(JSONKey.PLAYER.getKey()) + (currentBidderIndex + 1)
					+ jsonData.getString(JSONKey.WINNER.getKey()) + topBid);
	...
	public void offerProperty(StationSpace property, Player player) {
		boolean didChoose = gooey.getUserLeftButtonPressed(
				jsonData.getString(JSONKey.OFFER_PROPERTY.getKey()) + property.getBoardPosition()
		...
		if (didChoose) {
			gooey.showMessage(jsonData.getString(JSONKey.BOUGHT_PROPERTY.getKey()) + property.getName()
					+ jsonData.getString(JSONKey.PROPERTY.getKey()));
	...
	public void trade(Player trader) {
		if (!trader.getOwnedProperties(controller.getGame()).isEmpty()) {
			boolean trade = gooey.getUserLeftButtonPressed(
					jsonData.getString(JSONKey.TRADE_WTIH_ANOTHER_PLAYER.getKey()),
					jsonData.getString(JSONKey.YES.getKey()), jsonData.getString(JSONKey.NO.getKey()));
			while (trade) {
				boolean sell = gooey.getUserLeftButtonPressed(jsonData.getString(JSONKey.BUY_OR_SELL_PROPERTY.getKey()),
				...
				if (sell) {
					...
					String selection = JOptionPane
							.showInputDialog(null, jsonData.getString(JSONKey.WHICH_PROPERTY_DO_YOU_WANT.getKey()),
					...
					String playerToTrade = JOptionPane
							.showInputDialog(null, "Choose player to trade with.", "Sell property",
									JOptionPane.QUESTION_MESSAGE, null, otherPlayersNames, otherPlayersNames[0])
							.toString();
					Player tradee = nameToPlayer.get(playerToTrade);
					boolean accept = gooey.getUserLeftButtonPressed(tradee.getName() + ", do you accept the trade?",
							"Yes", "No");
					if (accept) {
					...
					String playerToTrade = JOptionPane
							.showInputDialog(null, "Choose player to trade with.", "Buy property",
									JOptionPane.QUESTION_MESSAGE, null, otherPlayersNames, otherPlayersNames[0])
							.toString();
					Player tradee = nameToPlayer.get(playerToTrade);
					...
					StationSpace selectedSpace = nameToOwnableSpace.get(selection);
					int amount = gooey.getUserInteger(
							trader.getName() + ", how much do you wish to pay for the property?", 0,
				...
				trade = gooey.getUserLeftButtonPressed(jsonData.getString(JSONKey.TRADE_WTIH_ANOTHER_PLAYER.getKey()),
						jsonData.getString(JSONKey.YES.getKey()), jsonData.getString(JSONKey.NO.getKey()));
			}
		}
	}
